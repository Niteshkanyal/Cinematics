...
import { View, TextInput, StyleSheet, Text, TouchableOpacity, ActivityIndicator, FlatList, Dimensions } from "react-native";
						...
						<TouchableOpacity onPress={() => { Actions.Tv_detail({ item: item }); }}>
							<View style={styles.flatlist_frame}>
								...
								<View style={styles.inner_container}>
									...
									<View style={{ flex: 0.1 }}>
										<Text style={styles.text_date}>{new Date(item.first_air_date).getFullYear()}</Text>
									</View>
									<View style={{ flex: 0.15, flexWrap: "wrap" }}>
										<Text style={styles.text_name} numberOfLines={1}>{item.name}</Text>
									</View>
									...
									<View style={{ flex: 0.35, flexDirection: "row" }}>
										...
										<View style={{ flex: 0.1 }}>
											<Text style={styles.text_voteavg}>{item.vote_average}</Text>
										</View>
